...
function phanTich_URL_chiTietSanPham() {
    ...
    // Đổi title
    document.title = nameProduct + ' - Macbook Sài Gòn';
    ...
    s += addThongSo('Màu sắc', sanPhamHienTai.detail.rom);
    s += addThongSo('Số lần sạc', sanPhamHienTai.detail.microUSB);
    ...
    // Hình nhỏ
    addSmallImg("img/products/logo.jpg");
    addSmallImg("img/products/logo.jpg");
    addSmallImg("img/products/logo.jpg");
...
function getDetailPromo(sp) {
    switch (sp.promo.name) {
        ...
        case 'moiramat':
            return `Khách hàng sẽ được thử máy miễn phí tại cửa hàng. Có thể đổi trả lỗi trong vòng 7 ngày.`;
        ...
        default:
            var span = `<span style="font-weight: bold">Túi Chống Sốc SIMTOP</span>`;
            return `Cơ hội nhập được ` + span + ` khi mua hàng `;
    }
}
